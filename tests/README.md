...
To verify a build of this tree instead of a published image:

```sh
rockcraft pack
sudo rockcraft.skopeo --insecure-policy copy \
  oci-archive:ps-printer-app_*.rock docker-daemon:ps-printer-app:local
IMAGE=ps-printer-app:local RUNTIME=podman tests/core-payload.sh
```

The FSDK image graph replaces the Rockcraft path. The verification interface is
unchanged: `just verify` runs `tests/core-appliance.sh` and `tests/core-payload.sh`
(see `docs/fsdk-ci.md`). The manifest, not this script, is what has to be updated
when that graph changes where the payload lands.
...
has to read to see what the image is expected to contain. The Docker image's
PostScript-only scope is the one exclusion the upstream source itself makes: `gs` is
built with `--with-drivers=ps2write`, and the non-PostScript Foomatic manufacturer
PPDs are removed before the archive is generated.